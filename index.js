...
const processDay = require('./lib/process-day');
...
function processLogs(config, cb) {
	...
	if (config.cfids && config.cfids.length > 0) {
...
function processDistribution(config, cfid, cb) {
	...
	async.during(
		(tCb) => probe(config, prefix, null, tCb),
		(jobCb) => processDay(config, listObjectResults, jobCb),
		cb
	);
}
...
function probe(config, prefix, token, cb) {
	...
	s3.listObjectsV2(listObjectsConfig, (err, data) => {
		...
		if (data.Contents.length === 0 && !data.IsTruncated) {
