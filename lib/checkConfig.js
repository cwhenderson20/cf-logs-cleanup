...
function checkConfig(config) {
	...
	if (!config.s3 && !config.aws.accessKeyId && !config.aws.secretAccessKey) {
		throw new Error("Either s3 or aws.accessKeyId and aws.secretAccessKey is required");
	}

	if (config.hasOwnProperty("partialMatching") && typeof config.partialMatching !== "boolean") {
	...
	if (config.cfids) {
		...
		if (Array.isArray(config.cfids)) {
			if (!config.cfids.every((cfid) => typeof cfid === "string")) {
				throw error;
			}
		} else if (typeof config.cfids !== "string") {
			throw error;
		} else {
	...
	if (config.hasOwnProperty("maxWorkers") && typeof config.maxWorkers !== "number") {
		throw new Error("Parameter 'maxWorkers' must be type number");
	}

	if (config.hasOwnProperty("daysToKeep") && typeof config.daysToKeep !== "number") {
