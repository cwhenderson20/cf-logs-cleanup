...
const debug = require('debug')('cf:worker');
...
process.on('message', ({ prefix, cfid, dateParts, nextHour }) => {
	async.auto({
		getHourPartsList: (cb) => getHourPartsList({ prefix, cfid, dateParts, nextHour }, cb),
		...
		clean: ['unzip', clean],
		rezip: ['clean', rezip],
		upload: ['rezip', (results, cb) => upload({ prefix, cfid, dateParts, nextHour }, results, cb)],
...
function getHourPartsList({ prefix, cfid, dateParts, nextHour }, cb) {
	const Prefix = constructS3DownloadPrefix(prefix, cfid, dateParts, nextHour);
	debug('Getting list of hour parts');
...
function upload({ prefix, cfid, dateParts, nextHour }, results, cb) {
	const keys = results.getHourPartsList;
	...
	const s3UploadPath = constructS3UploadPath({ prefix, cfid, dateParts, nextHour });
...
function constructS3DownloadPrefix(prefix, cfid, dateParts, nextHour) {
	const folder = prefix ? `${prefix.split('/').shift()}/` : '';
	return `${folder}${cfid}.${dateParts[0]}-${dateParts[1]}-${dateParts[2]}-${padHour(nextHour)}.`;
}

function constructS3UploadPath({ prefix, cfid, dateParts, nextHour }) {
	const folder = prefix ? `${prefix.split('/').shift()}/` : '';
	return `${folder}processed/${cfid}/${dateParts[0]}/${dateParts[1]}/${dateParts[2]}/${padHour(nextHour)}.gz`;
}
